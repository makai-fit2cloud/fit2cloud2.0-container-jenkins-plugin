...
public class F2CCodeDeployPublisher extends Publisher {
    ...
    private final boolean imageChecked;
    private final boolean containerAutoDeploy;
    ...
    public F2CCodeDeployPublisher(String f2cEndpoint,
        ...
        this.containerAutoDeploy = containerAutoDeploy;
        ...
        this.imageChecked = artifactType.equals(ArtifactType.IMAGE) ? true : false;
...
//        try {
        ...
        if (artifactType.equalsIgnoreCase(ArtifactType.IMAGE)) {
            try {
                String resourcePath = Utils.replaceTokens(build, listener, this.imageAddress);
                if (StringUtils.isBlank(resourcePath)) {
                    log("注册失败！未填写镜像地址，无法注册应用版本");
                } else {
                    String versionName = resourcePath.substring(resourcePath.lastIndexOf("/",resourcePath.lastIndexOf("/")-1)+1);
                    applicationVersion.setName(versionName);
                    applicationVersion.setAutoDeploy(this.containerAutoDeploy);
                    applicationVersion.setDeployType("container");
                    applicationVersion.setResourcePath(resourcePath);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
